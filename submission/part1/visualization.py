...
def compare_all_levels_training(results, save_path=None):
    """Create a single figure with subplots comparing all algorithm variants for all levels.
    ...
    fig.suptitle("Q-Learning vs SARSA (with/without Intrinsic Rewards): All Default Levels", fontsize=14, fontweight='bold')
    ...
    for idx, result in enumerate(results):
        ...
        for key, label, color, linestyle in variants:
            ...
            if stats and len(stats.episode_rewards) > 10:
                ma = stats.get_moving_average(stats.episode_rewards)
        ...
        # Add success rate annotation for all 4 variants
        annotations = []
    ...
    if save_path:
        ...
        print(f"Saved all levels comparison to {save_path}")
